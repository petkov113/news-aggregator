...
import { IArticle, APIResponseType, Category } from "./../reducers/types";

export const requestArticles = (
  category: Category = "all",
  keyword?: string
): ThunkAction<Promise<void>, RootState, unknown, ActionTypes> => async (
  dispatch,
  getState
) => {
  ...
  category !== "all"
    ? (url = `top-headlines?country=${state.feed.country}&category=${category}&pageSize=100&apiKey=${process.env.REACT_APP_API_KEY}`)
    : keyword
    ? (url = `everything?q=${keyword}&pageSize=30&sortBy=relevancy&apiKey=${process.env.REACT_APP_API_KEY}`)
    : (url = `top-headlines?country=${state.feed.country}&pageSize=100&apiKey=${process.env.REACT_APP_API_KEY}`);
  ...
  try {
    ...
    switch (e.response.status) {
      case 429:
        dispatch(
          setError("Our server is overloaded. Please, try again later.")
        );
        break;
      case 500:
...
const setArticles = (articles: IArticle[]): ActionTypes => {
  return {
    type: SET_ARTICLES,
    payload: articles,
  };
